...
def OurModel(input_shape, action_space):
    ...
    model = Model(inputs=X_input, outputs=X, name='Racer-DDQN-model')
...
class DQNAgent:
    ...
    def replay(self):
        ...
        target_val = self.target_model(next_state, training=False).numpy()
    ...
    def test(self):
        ...
        for e in range(self.EPISODES):
            ...
            while not done:
                self.env.render()
                action = np.argmax(self.model(state, training=False).numpy())
                next_state, reward, done, _ = self.env.step(action)
